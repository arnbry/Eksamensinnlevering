...
public:
    ...
    QSharedPointer<AssImpLoader::Node> getNodeData() { return m_rootNode; }

    // Texture information
    int numUVChannels() { return m_textureUV.size(); }
    int numUVComponents(int channel) { return m_numUVComponents.at(channel); }
private:
    QSharedPointer<AssImpLoader::MaterialInfo> processMaterial(aiMaterial *mater);
    QSharedPointer<AssImpLoader::Mesh> processMesh(aiMesh *mesh);
    void processNode(const aiScene *scene, aiNode *node, AssImpLoader::Node *parentNode, AssImpLoader::Node &newNode);

    void transformToUnitCoordinates();
    void findObjectDimensions(AssImpLoader::Node *node, QMatrix4x4 transformation, QVector3D &minDimension, QVector3D &maxDimension);
    ...
    QVector<QSharedPointer<AssImpLoader::MaterialInfo> > m_materials;
    QVector<QSharedPointer<AssImpLoader::Mesh> > m_meshes;
    QSharedPointer<AssImpLoader::Node> m_rootNode;
