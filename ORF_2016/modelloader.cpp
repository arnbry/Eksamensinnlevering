...
QString ModelLoader::findFile(QString relativeFilePath, int scanDepth)
{
...
bool ModelLoader::Load(QString filePath, PathType pathType)
{
    ...
    if(scene->HasMaterials())
    {
        ...
        {
            QSharedPointer<AssImpLoader::MaterialInfo> mater = processMaterial(scene->mMaterials[ii]);
...
//        qDebug() << "Has Lights";
    ...
    {
        AssImpLoader::Node *rootNode = new AssImpLoader::Node;
...
QSharedPointer<AssImpLoader::MaterialInfo> ModelLoader::processMaterial(aiMaterial *material)
{
    QSharedPointer<AssImpLoader::MaterialInfo> mater(new AssImpLoader::MaterialInfo);
...
QSharedPointer<AssImpLoader::Mesh> ModelLoader::processMesh(aiMesh *mesh)
{
    QSharedPointer<AssImpLoader::Mesh> newMesh(new AssImpLoader::Mesh);
...
void ModelLoader::processNode(const aiScene *scene, aiNode *node, AssImpLoader::Node *parentNode, AssImpLoader::Node &newNode)
{
    ...
    {
        QSharedPointer<AssImpLoader::Mesh> mesh = m_meshes[node->mMeshes[imesh]];
...
//    }
    ...
    {
        newNode.nodes.push_back(AssImpLoader::Node());
...
void ModelLoader::findObjectDimensions(AssImpLoader::Node *node, QMatrix4x4 transformation, QVector3D &minDimension, QVector3D &maxDimension)
{
