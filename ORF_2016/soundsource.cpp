...
bool SoundSource::loadWave(std::string filePath)
{
    ...
    if (!FileHandler::loadWave(filePath, waveData))
    {
...
void SoundSource::drawGeometry()
{

}

int SoundSource::initGeometry()
{
    return 0;
}
...
void SoundSource::setPosition(QVector3D newPos)
{
    mPosition = newPos;
    ALfloat temp[3] = {mPosition.x(), mPosition.y(), mPosition.z()};
    alSourcefv(mSource, AL_POSITION, temp);
}
void SoundSource::setVelocity(QVector3D newVel)
{
    mVelocity = newVel;
